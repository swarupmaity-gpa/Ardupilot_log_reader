...
if __name__ == "__main__":
	...
	csv_log = open(filename+"_elec_1.csv","w",newline='')
	...
	while True:
		...
		try:
			...
			if data_packet['mavpackettype']=='CAM':
				print("RAW data",data_packet)
			#print("RAW",data_packet)
			
			try:
				if data_packet['mavpackettype'] == 'CAM':
					for i in data_packet:
						if type(data_packet[i])!=str:
							copy_buffer.append(data_packet[i])
					print(copy_buffer)
					print("RAW",data_packet)
					writer.writerow(copy_buffer)
			except:
				pass
			
			#csv_log.write(copy_buffer)
			#print(copy_buffer)
...
"""
{'mavpackettype': 'FMT', 'Type': 167, 'Length': 37, 'Name': 'BAT', 'Format': 'Qfffffcf', 'Columns': 'TimeUS,Volt,VoltR,Curr,CurrTot,EnrgTot,Temp,Res'}
"""
